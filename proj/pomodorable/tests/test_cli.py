...
@pytest.mark.parametrize("subdir_param", [None, "exported"])
def test_export_csv_for_date_range_mutant(
    app_data_with_four_test_sessions, subdir_param, monkeypatch
):
    app_data, _ = app_data_with_four_test_sessions
    data_dir = str(app_data.data_path)
    ...
    if subdir_param:
        ...
        export_path = app_data.data_path
        args = ["--csv-date", "2024-01-02", "--end-date", "2024-01-02"]

    date1 = datetime.strptime(args[1], date_format)
    date2 = datetime.strptime(args[3], date_format)
    
    runner = CliRunner()
    result = runner.invoke(cli, args)
    print(result.output)
    assert date1 == date2 and result.output != "\nStart date must be before end date.\n"
    assert result.exit_code == 0
